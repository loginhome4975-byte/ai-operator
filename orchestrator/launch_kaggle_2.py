    ...
    with open(env_path, "r") as f:
        for line in f:
            ...
            if line and not line.startswith("#") and "=" in line:
                ...
                if key.strip() == "KAGGLE_USERNAME_2":
                    os.environ["KAGGLE_USERNAME"] = val
                elif key.strip() == "KAGGLE_KEY_2":
...
if not kaggle_user:
    print("XATO: .env faylida KAGGLE_USERNAME_2 topilmadi!")
    sys.exit(1)
...
dataset_slug = f"{kaggle_user}/ai-operator-node2-venv"
dataset_name = "ai-operator-node2-venv"
...
def keep_registering(url):
    ...
    while True:
        try:
            ...
            # Har bir muvaffaqiyatli register'da venv upload (flag fayli bor bo'lsa)
            if r.status_code == 200:
...
# --- CODE HASH: pip_commands + main_app_code ---
# Delta detection: agar hash o'zgarsa → deltadata=true, eski dataset ishlatilmaydi
code_hash = hashlib.sha256(
    ("\n".join(pip_commands) + encoded_main).encode()
).hexdigest()[:16]

# --- .env dan oldingi ma'lumotlarni o'qish ---
stored_hash = os.environ.get("NODE2_VENV_HASH", "")
stored_path = os.environ.get("NODE2_DATASET_PATH", "")
...
# dataset_exists - Kaggle API o'rniga .env'dan o'qiladi
...
print("Kaggle Node 2 initialization starting...")
...
if not DELTADATA == "true" and os.path.isdir(DATASET_PATH):
    ...
    if os.path.exists(hash_file) and os.path.exists(tar_file):
        ...
        if cached_hash == VENV_HASH:
            ...
            print(f"Hash mismatch. Venv qayta quriladi.")
    else:
...
if not use_cache:
    ...
    # Flag fayli — main_app tunnel ochilgach orchestrator'ga yuboradi
    upload_info = {{
        "tar_path": tar_path,
        "dataset_slug": DATASET_SLUG,
        "dataset_name": DATASET_NAME,
...
print("[2/2] Server venv da ishga tushirilmoqda...")
...
# dataset_sources: .env'dan path o'qiladi, deltadata bo'lsa qo'shilmaydi
dataset_sources = []
if dataset_exists:
    dataset_sources.append(stored_path)

metadata = json.dumps({
    "id": f"{kaggle_user}/ai-operator-kaggle-node-2",
    "title": "ai-operator-kaggle-node-2",
...
print("Kaggle 2 Kernel yuborildi!")
